...
export default function Home() {
  ...
  return (
    <main className="font-sans min-h-screen mx-auto bg-gradient-to-br from-indigo-500 to-purple-600 p-4 md:p-6 max-w-full">
      <div className="bg-white/95 backdrop-blur-lg shadow-2xl rounded-xl md:rounded-2xl p-5 md:p-8 max-w-6xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 p-6 rounded-xl border border-blue-200">
              <ol className="list-decimal list-inside space-y-3 text-sm">
                <li>
                  <strong className="text-blue-800">Promise 생성:</strong> 각
                  API 호출은 서로 다른 지연 시간(1~5초)을 가진 Promise로
                  생성됩니다.
                </li>
                <li>
...
const promises = apiConfigs.map((config) => {
  return fakeApi(config.id, config.delay).then((result) => {
    ...
    return result;
                ...
                <p className="text-sm text-gray-600 mt-4 p-3 bg-blue-50 rounded-lg">
                  <strong>핵심:</strong> 모든 Promise가 동시에 시작되지만,
                  각각의 지연 시간에 따라 완료 순서가 달라집니다. 위 예제에서는
                  API 2 → API 1 → API 3 순서로 완료됩니다.
                </p>
              </div>
